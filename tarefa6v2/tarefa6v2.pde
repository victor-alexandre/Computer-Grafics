   ...
   // ObjectList.add(new Objeto3D(cubo, arestaCubo,500,500,500));    
    ...
    for(int i = 0; i < qtdObjetos_na_figura; i++){
        ...
        for(int k = 0, j = contadorLinha+qtdpontos+qtdlinhas+1; j <= contadorLinha+qtdpontos+qtdlinhas+qtdfaces; j++, k++){             
            ...
            int []Figure_Face_Points = new int[parts.length];
            float R,G,B;
            for(int m = 0; m < parts.length - 3  ; m++){
                Figure_Face_Points[m] = Integer.parseInt(parts[m])-1; 
                println(Figure_Face_Points[m]+" ");
            }

            R = Float.parseFloat(parts[parts.length - 3]); 
            G = Float.parseFloat(parts[parts.length - 2]);
            B = Float.parseFloat(parts[parts.length - 1]);
            println(R + " " + G + " " + B);
...
void draw(){
    ...
    if(menu){
        ...
        for(int i = 0; i < ObjectList_Com_Faces.size(); i++){ //<>//
            if(i == isSelected)ObjectList_Com_Faces.get(i).desenhaObjeto3D(true);
            else ObjectList_Com_Faces.get(i).desenhaObjeto3D(false);
        }     //<>//
    } //<>//
}
