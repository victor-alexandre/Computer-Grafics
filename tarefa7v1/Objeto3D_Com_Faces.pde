public class Objeto3D_Com_Faces{
    ...
    void desenhaObjeto3Dcolorido(boolean isSelected){
        ...
        if(isSelected){
            for(int i = 0; i < FaceList.size(); i++){
                PoligonoVF faceatual = new PoligonoVF(Pontos, L, FaceList.get(i), color(0,255,0), true, color(0,0,255));
            }
        }
        else{
            for(int i = 0; i < FaceList.size(); i++){
                PoligonoVF faceatual = new PoligonoVF(Pontos, L, FaceList.get(i), color(255,0,0), true, color(0,0,255));
            }
        }
    }
